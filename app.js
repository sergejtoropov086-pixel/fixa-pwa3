...
// Инициализация SQLite в браузере через CDN
async function initDatabase() {
  try {
    document.getElementById('app').innerHTML = 'Инициализация базы данных...';
    
    // Загружаем WASM-модуль SQLite через CDN
    const SQL = await initSqlJs({
      locateFile: filename => `https://cdnjs.cloudflare.com/ajax/libs/sql.js/2.0.1/${filename}`
    });
    ...
    document.getElementById('app').innerHTML = `
      <div class="card">
        ...
        <p>Проверьте подключение к интернету при первом запуске.</p>
...
function render() {
  ...
  if (currentView === 'home') {
    html = `
      ...
      <p class="status">Данные загружены локально. Интернет не требуется после первого запуска.</p>
    `;
